...
WS.onEvent(ev => {
  ...
  switch (ev.type) {
    ...
    case "background_tasks_update":
      // Snapshot of in-flight fire-and-forget tasks for this session.
      // Renders a small badge in #session-info-bar (running > 0) or hides it.
      if (ev.session_id !== Sessions.activeId) break;
      Sessions.updateBgTasksBadge(ev.running || 0, ev.tasks || []);
      break;

    case "background_task_notice":
      // Transition bubble emitted right before the agent processes a
      // completed background task. Without this, the next assistant message
      // looks like the agent is talking to itself.
      if (ev.session_id !== Sessions.activeId) break;
      Sessions.appendBgTaskNotice(ev.command, ev.task_id, ev.status);
      break;
