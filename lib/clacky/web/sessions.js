...
const Sessions = (() => {
  ...
  return {
    ...
    /**
     * Render the background-task badge in #session-info-bar.
     *
     * - running > 0 → shows "🔄 N background task(s)" with rotating dot
     * - running = 0 → hides the badge entirely
     *
     * Hovering the badge reveals a tooltip with command + elapsed-time of
     * each task. Click toggles a popover with the same info (touch-friendly).
     */
    updateBgTasksBadge(running, tasks) {
      ...
      // Tooltip — show first 3 task commands with elapsed time.
      ...
      // Stash for popover render
      badge.dataset.tasks = JSON.stringify(tasks || []);
    },

    /**
     * Append a "transition bubble" line indicating that the agent is about
     * to react to a completed background task. Kept very compact so it
     * doesn't visually compete with real assistant messages.
     */
    appendBgTaskNotice(command, taskId, status) {
      Sessions.collapseToolGroup();
      const messages = $("messages");
      const el = document.createElement("div");
      ...
      // Show short id + short command — full command lives in the tooltip
      const cmdShort = (command || "").length > 60
        ? (command.slice(0, 60) + "…")
        : (command || "");
      const idSuffix = taskId ? `  <code class="bgtask-id">${escapeHtml(taskId)}</code>` : "";
